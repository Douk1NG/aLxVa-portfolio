    ...
    <div className="flex flex-col lg:grid lg:grid-cols-12 gap-0 lg:min-h-125">
      ...
      <div className="lg:col-span-7 p-5 sm:p-8 md:p-10 flex flex-col gap-6 sm:gap-8 border-b lg:border-b-0 lg:border-r border-border/50">
        <header className="space-y-4 sm:space-y-6">
          <div className="space-y-3 sm:space-y-4">
            <div className="space-y-1 sm:space-y-2">
              <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-extrabold text-foreground tracking-tight leading-tight">
                {props.title}
              </h2>
              {props.subtitle && (
            ...
            {props.metaItems &&
              props.metaItems.length > 0 && (
                <div className="flex flex-wrap gap-x-4 sm:gap-x-6 gap-y-2 text-[10px] sm:text-xs md:text-sm text-muted-foreground font-medium pt-1">
        ...
        <section className="space-y-3 sm:space-y-4 flex-1">
          <h3 className="text-[10px] sm:text-xs md:text-sm font-bold uppercase tracking-[0.2em] text-primary/80 flex items-center gap-2">
            {props.descriptionLabel}
          </h3>

          <div className="text-sm sm:text-base md:text-lg text-foreground/80 leading-relaxed whitespace-pre-line bg-muted/20 p-5 sm:p-6 rounded-2xl border border-border/30 shadow-sm">
            {props.description}
          </div>
        </section>
      </div>
